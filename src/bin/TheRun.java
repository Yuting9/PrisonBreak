...
public class TheRun extends JPanel implements ActionListener, KeyListener
{
 ...
 int    plTime   = 0,
           doTime = 0, prTime = 0, buildCount = 0, x = 720, xCoor = 0, dHeight = 0, change, spd, distance;
 ...
 public TheRun()
 {
  ...
  /*
   * for (int d = 0; d < 4; d++) { onDonut[d] = new Sprite(-100 + (210*d)); }
   */
  for (int i = 0; i < 6; i++)
  {
 ...
 public void actionPerformed(ActionEvent e)
 {
  ...
  else
  {
   ...
   for (int i = 0; i < 5; i++)
   {
    ...
    if (onDonut[i].getX() > player.getX() && onDonut[i].getX() < player.getX() + 100
      && onDonut[i].getY() + 5 > player.getY() && onDonut[i].getY() < player.getY() + 100 && !onDonut[i].getEaten())
    {
     onDonut[i].eaten();
    }
   }
   if (onCoffee.getX() > player.getX() && onCoffee.getX() < player.getX() + 100
