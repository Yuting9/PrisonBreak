 ...
 * Description: The main running file, leading to score input output, containing
 * 							the whole flesh of the game
 *
 * Author: Yuting L.
...
public class TheRun extends JPanel implements ActionListener, KeyListener
{
	...
	// The main menu to be returned to
	JFrame menu;
	...
	// The prison image
	ImageIcon imgPris = new ImageIcon(TheRun.class.getResource("/img/prison.png"));
	
	// The x value for the prison image
	int x = 0;
	
	// Game Class
	public TheRun(JFrame main)
	{
		menu = main;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else
		{
			...
			if (prison.getX() <= player.getX())
			{
				...
				Victory win = new Victory(true, menu);
			...
			else if (prison.getX() > 900)
			{
				...
				Victory win = new Victory(false, menu);
	...
	class DrawPanel extends JPanel
	{
		...
		private void paintBack(Graphics g)
		{
			...
			// If the prison is on screen, show it.
			if(x>-1000)
			{
				imgPris.paintIcon(this, g, x, 0);
			}
		...
		/**
		 * Paints the distance from prisoner and current points
		 * 
		 * @param g
		 *          - The graphics variable
		 */
